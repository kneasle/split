...
"""
Build script for Split's webpage.

Largely copied from Jigsaw's build script:
https://github.com/kneasle/jigsaw/blob/df33762d5e8b7216fb13a5e63819a7b0ab8881d4/build.py

Installation requirements:
- rustup target add wasm32-unknown-unknown
- cargo install wasm-gc wasm-bindgen-cli
"""
...
RUST_CRATE_NAME = "split"
RUST_CRATE_PATH = ""  # Path to the directory containing `Cargo.toml`
...
for file_name in os.listdir(web_dir):
    ...
    if os.path.exists(out_path):
        os.remove(out_path)
    # Copy the source file to the out dir
    if is_release:
